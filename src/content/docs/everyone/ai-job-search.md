---
title: "AI for Job Search: Resumes, Cover Letters, Interview Prep"
description: "Use AI to optimize your job search, craft applications, and prepare for interviews"
sidebar:
  badge:
    text: "Intermediate"
    variant: caution
version: "1.0"
---

# AI for Job Search

## Resume Optimization

### Tailoring to Job Description

```
Prompt: Optimize my resume for this job:

My resume: [paste resume]

Job description: [paste JD]

Tailor my:
- Summary
- Skills section
- Experience descriptions
- Keywords for ATS

Maintain truthfulness.
```

### ATS-Friendly Formatting

```
Prompt: Review my resume for ATS compatibility:

[paste resume]

Check for:
- Parsing issues
- Keyword optimization
- Format problems
- Missing sections
```

## Cover Letter Generation

```
Prompt: Write a cover letter:

Job: [title] at [company]
Key requirements: [from JD]
My relevant experience:
- [Experience 1]
- [Experience 2]
- [Experience 3]

Tone: Professional but personable
Length: 250-300 words
Highlight: why I'm a great fit
```

## LinkedIn Optimization

### Profile Summary
```
Prompt: Write LinkedIn summary:

Role: [Your title]
Experience: [Years]
Specialties: [Your skills]
Achievements: [Key wins]
Looking for: [Job type]

Make it: Engaging, keyword-rich, 150 words
```

### Post Ideas
```
Prompt: Generate 5 LinkedIn post ideas about:
- Job searching
- [Your industry]
- Professional development

Include: Hook, value, call-to-action
```

## Interview Preparation

### Company Research
```
Prompt: Research brief for interview:

Company: [Company name]
Industry: [Industry]
Role: [Position]

Provide:
- Company overview
- Recent news
- Competitors
- Culture insights
- Questions to ask
```

### Common Questions
```
Prompt: Help me prepare answers for:

1. "Tell me about yourself"
2. "Why this role?"
3. "Greatest strength/weakness"
4. "Where do you see yourself in 5 years?"

Based on:
- Role: [Job title]
- My background: [Summary]
- Company: [Company name]

Use STAR method for examples.
```

### Mock Interview
```
Prompt: Conduct mock interview:

Role: [Position]
Industry: [Industry]
Experience: [My level]

Ask me 5 interview questions one at a time.
After each answer, provide feedback on:
- Strengths
- Areas to improve
- Suggested revisions
```

## Salary Negotiation

```
Prompt: Help me negotiate salary:

Offer: $[amount]
Market rate: $[range]
My experience: [Years]
Location: [City]
Benefits: [Current offer]

Provide:
- Counter-offer suggestion
- Negotiation script
- Alternative asks if salary fixed
- Email template
```

## Networking Messages

### LinkedIn Connection
```
Prompt: Write LinkedIn connection message:

To: [Name], [Title] at [Company]
Context: [How you found them]
Common ground: [Shared interest/experience]
Goal: [What you hope for]

Length: Under 300 characters
Tone: Professional, genuine
```

### Informational Interview
```
Prompt: Request informational interview:

To: [Name]
About: [Their role/company]
Why: [Your interest]
Ask: 15-20 minute call

Make it easy to say yes.
```

## Application Tracking

### Organization System
```
Prompt: Create job search tracking system:

Track:
- Company
- Position
- Date applied
- Status
- Follow-up dates
- Contacts
- Notes

Format: Spreadsheet structure
```

## Email Templates

### Thank You Email
```
Prompt: Write interview thank you email:

Interviewer: [Name]
Position: [Role]
Interview date: [Date]
Key discussion points:
- [Point 1]
- [Point 2]

Include:
- Gratitude
- Reiterate interest
- Reference specific conversation
- Next steps

Length: 150-200 words
```

### Follow-Up
```
Prompt: Write follow-up email:

Situation: Applied [X] weeks ago, no response
Company: [Company name]
Position: [Role]

Goal: Gentle reminder
Tone: Professional, not pushy
Length: Under 100 words
```

## Job Description Analysis

```
Prompt: Analyze this job description:

[Paste JD]

Extract:
- Must-have skills
- Nice-to-have skills
- Company culture indicators
- Red flags
- Questions to ask
- Salary range estimate
```

## Portfolio/Projects

```
Prompt: Describe this project for my portfolio:

Project: [Name]
What it does: [Description]
Technologies: [List]
My role: [Your contribution]
Outcome: [Results]

Write:
- Engaging title
- 2-paragraph description
- Key achievements
- Technical details

Audience: Hiring managers
```

## Best Practices

✅ **Do**:
- Personalize AI output
- Fact-check everything
- Maintain authenticity
- Use as starting point
- Practice answers out loud

❌ **Don't**:
- Copy-paste without editing
- Fabricate experience
- Use generic applications
- Rely solely on AI
- Skip proofreading

---

**Found an issue?** [Open an issue](https://github.com/javirub/The-New-Era-Codex/issues)!
